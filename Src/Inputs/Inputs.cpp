...
bool CInputs::Poll(const Game *game, unsigned dispX, unsigned dispY, unsigned dispW, unsigned dispH)
{
	...
	std::map<const char *, const char *> translate_both_keys = {
		{"Start1", "Start2"},
		{"Coin1", "Coin2"},
		{"ServiceA", "ServiceB"},
		{"TestA", "TestB"},
	...
	std::map<const char *, uint16_t> mod_player_inputs = {
		{"Start1", 0},
		{"Coin1", 0},
		{"ServiceA", 0},
		{"TestA", 0},
	...
	std::map<const char *, uint16_t> blank_p1_inputs = {
		{"Start1", 0},
		{"Coin1", 0},
		{"ServiceA", 0},
		{"TestA", 0},
	...
	if (Dojo::players_swapped || Dojo::Replay::p2_override)
	{
		for (vector<CInput*>::iterator it = m_inputs.begin(); it != m_inputs.end(); ++it)
		{
			...
			{
				(*it)->Poll();
				mod_player_inputs[swapped_player_inputs[(*it)->id]] = (*it)->value;
			}
		}
	}

	Dojo::Poll::StartAction();
	for (vector<CInput*>::iterator it = m_inputs.begin(); it != m_inputs.end(); ++it)
	{
		...
		if (Dojo::Replay::p1_override && blank_p2_inputs.count((*it)->id))
			(*it)->value = blank_p2_inputs[(*it)->id];

		if (Dojo::Replay::p2_override && blank_p1_inputs.count((*it)->id))
			(*it)->value = blank_p1_inputs[(*it)->id];
